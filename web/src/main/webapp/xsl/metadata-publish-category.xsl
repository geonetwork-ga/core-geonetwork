								...
								<xsl:if test="not($disabled)">
									<tr width="100%">
										<td align="center" colspan="2">
											<xsl:choose>
												<xsl:when test="contains(/root/gui/reqService,'metadata.batch.publish')">
													<button class="content" onclick="checkBoxModalUpdate('categories','metadata.batch.publish','true','{concat(/root/gui/strings/results,' ',/root/gui/strings/batchPublishTitle)}')"><xsl:value-of select="/root/gui/strings/submit"/></button>
												</xsl:when>
												<xsl:otherwise>
													<button class="content" onclick="checkBoxModalUpdate('categories','metadata.publish');"><xsl:value-of select="/root/gui/strings/submit"/></button>
												</xsl:otherwise>
											</xsl:choose>
										</td>
									</tr>
